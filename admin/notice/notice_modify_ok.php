<?php
  ...
  if($_FILES['file']['name']){
    ...
    if(isset($_FILES['file'])){
      //파일 업로드
      $save_dir = $_SERVER['DOCUMENT_ROOT']."/attention_renewal/pdata/notice/"; //파일 저장 경로
      ...
      if(move_uploaded_file($_FILES['file']['tmp_name'], $save_dir.$notice_file)){  
        $notice_file = "/attention_renewal/pdata/notice/".$notice_file;
      } else{
        echo "<script>
          alert('파일 첨부 실패.. :(');    
          history.back();            
  ...
  if($mysqli->query($sql) === true){
    echo "<script>
      alert('수정 완료되었습니다 :)');
      location.href='/attention_renewal/admin/notice/notice.php';</script>";
  } else{
    echo "<script>
      alert('수정 실패.. :(');
      history.back();</script>";
